...
export const HardModeGamePage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const hasSeenTooltip = sessionStorage.getItem('wordle-hard-help-tooltip-seen');
    if (!hasSeenTooltip) {
      setShowHelpTooltip(true);
      const timer = setTimeout(() => {
        setShowHelpTooltip(false);
        sessionStorage.setItem('wordle-hard-help-tooltip-seen', 'true');
      }, 5000);
  ...
  const handleEnter = useCallback(() => {
    ...
    // Validar hard mode rules
    if (attempts.length > 0) {
      const hardModeValidation = validateHardMode(guessWord, attempts);
      if (!hardModeValidation.isValid) {
        // Animación shake en la fila actual
        const currentRowIndex = attempts.length;
        setShakingRow(currentRowIndex);
        setToast({ 
          message: hardModeValidation.errorMessage || 'Hard mode rule violated', 
          type: 'error' 
        });
  ...
  }, [targetWord, currentGuess, gameStatus, dictionaryReady, attempts, validateWordLocal, getDictionary, submitGuess, keyboardReadyRow]);
